...
function ItineraryDayCard({ day, isCollapsed, onToggleCollapse, onFieldChange, onAddItem, onRemove, onToggleEdit, isEditing }) {
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mb-4">
      ...
      {!isCollapsed && (
        <div className="p-4 space-y-3">
          {day.items.map((item) => (
            <ItineraryItem key={item.id} item={item} dayId={day.id} onFieldChange={handleFieldChange} onRemove={handleRemove} />
          ))}
...
// Child: ItineraryItem
function ItineraryItem({ item, dayId, onFieldChange, onRemove }) {
  return (
    <div className="group relative p-3 bg-slate-50 rounded-lg border border-slate-100 hover:border-indigo-200 transition-colors">
...
export default function ItineraryTab({ itinerary, setItinerary }) {
  ...
  const handleDrop = useCallback((dayId) => {
    if (draggedItemIndex !== null && dropTargetIndex !== null && dragDayIdRef.current === dayId) {
      handleReorderItems(dayId, draggedItemIndex, dropTargetIndex);
    }
    setDraggedItemIndex(null);
    setDropTargetIndex(null);
    setIsDragOver(false);
  }, [draggedItemIndex, dropTargetIndex, handleReorderItems]);
